...
function RenderChunkRenderer({ chunk }: { chunk: BackendRenderChunk }) {
  ...
  if (chunk.type === 'button') {
    const handleClick = () => {
      if (selectedChat) {
        // For now, we'll use mock content since we don't have content generation yet
        const chatContents = mockContents[selectedChat.id] || []
        const content = chatContents.find(c => c.id === chunk.option_id)
        if (content) {
...
export function MessageBubbleBackend({ message }: MessageBubbleBackendProps) {
        ...
        >
          <div className={`space-y-3 ${isUser ? 'text-black' : 'text-text'}`}>
            {renderChunks.map((chunk, index) => (
              <RenderChunkRenderer key={index} chunk={chunk} />
            ))}
          </div>
        </motion.div>
        <motion.div
